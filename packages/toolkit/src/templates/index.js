...
export function templateCategorySlug(value = '') {
  return (
    value
      .toString()
      .trim()
...
export function normaliseTemplateRow(row) {
  if (!row) return null;
  const categoryTitle = row.category || 'General';
  return {
    raw: row,
    categoryId: templateCategorySlug(categoryTitle),
    categoryTitle,
...
export function mapRowsToCollections(rows = [], fallbackCollections = TEMPLATE_COLLECTIONS) {
  if (!rows.length) return fallbackCollections;
  const byCategory = new Map();

  rows.forEach((rawRow) => {
    const normalised = normaliseTemplateRow(rawRow);
  ...
  const mapped = Array.from(byCategory.values()).map((collection) => ({
    ...collection,
    templates: collection.templates.sort((a, b) => (b.popularity || 0) - (a.popularity || 0))
  }));

  if (!mapped.length) return fallbackCollections;

  return mapped.sort((a, b) => (b.templates.length || 0) - (a.templates.length || 0));
}
